...
class Station(models.Model):
	...
	pref_code = models.ForeignKey(Prefecture, null=True, blank=True, on_delete=models.SET_NULL, verbose_name='都道府県')
...
class Name(models.Model):
	...
	name = models.CharField(max_length=200)
	creator = models.ForeignKey(Creator, on_delete=models.SET_NULL, null=True, blank=True, related_name='part_name', verbose_name='名義')
	def __str__(self):
		return self.name
...
class Movie(models.Model):
	...
	song = models.ManyToManyField('Song', blank=True, verbose_name='使用楽曲(作品全体)', related_name='entire_songs_in_movie')
	vocal = models.ManyToManyField('Vocal', blank=True, verbose_name='使用ボーカル(作品全体)', related_name='entire_vocals')
	COLLAB = (
		('S', '単作'),
		('C', '合作')
	)
...
class Part(models.Model):
	...
	part_song = models.ManyToManyField('Song', blank=True, verbose_name='使用楽曲(一部)', related_name='part_songs')
	part_vocal = models.ManyToManyField('Vocal', blank=True, verbose_name='使用ボーカル(一部)', related_name='part_vocals')
...
class StationInMovie(models.Model):
	...
	station_service_code = models.ForeignKey(StationService, to_field='station_service_code', on_delete=models.CASCADE, related_name='駅名')
	...
	def __str__(self):
		return self.station_service_name