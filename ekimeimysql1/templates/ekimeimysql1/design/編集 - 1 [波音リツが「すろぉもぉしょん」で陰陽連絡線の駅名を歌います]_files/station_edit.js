...
function namesearch() {
	...
	$.getJSON(s, function(data) {
		for (var i in data) {
			var op_station = "<option value='" + data[i].station_service_pk + "' data-name='" + data[i].__str__ + "' data-line='" + data[i].line_service_name + "' class='station-option station-search-option'>" + data[i].__str__ + " ‐ " + data[i].line_service_name + "</option>"
			$('.station-select').append(op_station)
		}
	})
}
