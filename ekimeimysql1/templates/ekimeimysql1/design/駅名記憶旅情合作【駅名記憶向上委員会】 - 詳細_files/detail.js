		...
		(function(id, category) {
			...
			$.getJSON(s, function(data) {
				...
				for (var i in data) {
					...
					// 	key_station = "<div class='key-station'><div class='key-station-icon'></div><div class='station-before' style='background: " + data[i].get_color + "'></div><div class='station-after' style='background: " + data[i].get_color + "'></div><a href='http://localhost:8000/ekimeimysql1/stationservice/" + data[i].station_service_pk + "'>" + station_text + "</a></div>"
					// 	line_box = "<div class='line-box'><div class='line-button'></div><a href='http://localhost:8000/ekimeimysql1/stationservice/" + data[i].line_service_pk + "'><div class='station-before' style='background: " + data[i].get_color + "'></div><div class='station-after' style='background: " + data[i].get_color + "'></div><div class='line-icon'><img src='static/channel.jpg'></div><div class='line-name'>" + data[i].line_service_name + "</div></a></div>"
					// 	$('.station-list-' + id).append(key_station + line_box + "<ul class='station-ul station-ul-" + forloop + "'></ul>");
					// } else {
					...
					// console.log(afterback);
						...
						ul++;
						console.log(data[i].station_sung_name)
...
$(document).ready(function() {
	$('.station-list').each(function(i, elem) {
		$(elem).find('.line-name').each(function(j, line) {
			console.log(i + " " + lin.text())
		})
	})
})